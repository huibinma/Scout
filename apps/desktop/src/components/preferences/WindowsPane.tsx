 ...
 * 2026-07-26：加「关闭到托盘」开关（settings.close_to_tray，Rust 侧 main.rs 的
 * CloseRequested 拦截 + tray.rs 消费）。
 */
        ...
        <label className="prefs-checkbox">
          ...
          <span>
            ...
            <span className="prefs-hint">
              ...
              继续在后台索引、监听全局快捷键；系统托盘保留一个常驻图标，随时可通过{" "}
              <code>{settings.global_shortcut}</code>{" "}
              或托盘图标唤起。托盘菜单里的「退出 Scout」才会真正退出程序。关闭本开关后，关窗恢复为直接退出。
            </span>
          </span>
        </label>
      </div>
