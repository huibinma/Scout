...
| ID | 标题 | 状态 | 模块 | 依赖 | 估时 |
|---|---|---|---|---|---|
| **BETA-00** | **开源发布审查（LICENSE / Third-party Notices / 隐私说明 / 商标使用规范 / 仓库脱敏）** | **done（2026-07-04）**：LICENSE 双许可 ✅ + Everything 条款核查 ✅（不构成再分发，[third-party-licenses.md](./docs/third-party-licenses.md)）+ PRIVACY.md ✅ + **仓库脱敏 ✅**（语料全合成核实 / 图片无 EXIF / 密钥工作区+924 commits 历史双零命中 / 个人用户名 148 处等长替换清零、受改 crate 测试全绿，报告见当时的脱敏核查记录）。**仓库已转公开（2026-07-04 同日，orphan 首发）**：原公开仓库自单 commit `bc47473` 起步、私有归档仓库冻结保全史（报告 §4 执行记录）——BETA-00 无开放项。**2026-07-24 起项目重构迁移至私有仓库 `huibinma/Scout`（Scout 品牌重命名 + 工程目录/文档精简），与原公开仓库各自独立演进**。**2026-08-08 `huibinma/Scout` 再度转公开**（同款 orphan 单提交先例复用）：改名归档为私有 `huibinma/scout-archive`（保留完整 122 提交历史）+ 新建同名 public 仓库（orphan 单提交 `c10f5e3` 起步，工作区 LociFind 时期内容/提交历史清零、作者身份改用 GitHub noreply 邮箱）；README 定位改写为"面向 agent 的本地文件搜索工具，经 MCP 接入 Claude Code/Codex 等，同时供人直接用" | docs/ | MVP-28 | 完成于 2026-07-04 |
...
| **BETA-73** | **全局唤起快捷键改为真正可自定义，默认改回 Ctrl+Space；与「关闭窗口时驻留系统托盘」联动禁用**（用户反馈：快捷键设置页是禁用输入框，且 BETA-72 当天改的 `Ctrl+Alt+S` 默认值用户明确要求改回 `Ctrl+Space`；另要求把托盘驻留开关与快捷键合并同一分区、逻辑联动） | **done（2026-08-08）**：**①可自定义**：新增 `shortcut::update_global_shortcut` 命令——校验格式（拒绝无修饰键的裸键组合，防全局吞掉某个键的全部正常输入）→ `unregister_all` 清场 → 重新 `on_shortcut` 注册 → 落盘；任一步失败（含被其他程序占用的系统级冲突）当场把原始错误返回前端、不落盘。**②默认值统一 `Ctrl+Space`**：跨平台不再区分 macOS/Windows；`settings.rs` 新增 `DEFAULT_GLOBAL_SHORTCUT` 常量 + `read_global_shortcut` live-read（缺字段/空串/解析失败均回退默认）。**③前端**：新增 `ShortcutRecorder.tsx`（点「修改」录制下一次按键组合、Esc 取消、「恢复默认」复位、失败当场提示）+ `lib/shortcut.ts`（`KeyboardEvent` → 内部格式的编解码，`ShortcutBanner` 与录制器共用，banner 不再按平台硬编码猜文案）。**④覆写保护**：`global_shortcut` 纳入 `settings::merge_backend_managed_fields`（同 MCP token/enabled 既有套路）——`update_global_shortcut` 绕开设置表单直接改磁盘现值，若不保护，表单挂载期旧快照的整体保存会把刚生效的新快捷键冲回去。**⑤设置页合并**：「关闭窗口时驻留系统托盘」从 `WindowsPane.tsx` 底部挪到 `GeneralPane.tsx` 顶部与快捷键合并一个分区，`shortcutDisabled = IS_WINDOWS && !settings.close_to_tray` 联动置灰快捷键录制器（关窗不驻留托盘＝进程真退出、快捷键唤不起已退出的程序；macOS 无此依赖，无该开关时快捷键恒可用）。**浏览器实测中发现并修复两个真实 bug**（均非纸面审查）：录制器对空 `KeyboardEvent.code`（某些合成/边缘输入场景）缺兜底会拼出 `"Ctrl+Alt+"` 畸形字符串——`eventToShortcut` 补 `!e.code` 早退；Esc 取消录制只 `preventDefault` 未 `stopPropagation`，导致按键继续冒泡触发外层设置面板自己的 Esc-关闭监听器、整个弹窗被一起关掉——录制期间统一改为进入 handler 即 `preventDefault`+`stopPropagation`。**验证**：Rust `cargo test -p scout-desktop` 197 全绿（新增 `shortcut::tests` 6 条 + `settings::tests` 3 条，含 `update_settings_preserves_backend_managed_global_shortcut` 覆盖标 ④ 的覆写保护）、`clippy -D warnings`/`fmt --check` 净；`tsc --noEmit`/`vite build` 全绿；浏览器预览注入 `window.__TAURI_INTERNALS__.invoke` stub，实测录制成功/系统级冲突报错（不落盘）/Esc 取消不关闭弹窗/托盘开关联动置灰四条路径。**未尽事宜**：均未做真机验证（尤其 Windows 上与其他常驻软件的实际快捷键冲突场景）；`docs/` 下用户文档未同步提及快捷键改为可配置。 | apps/desktop（`shortcut.rs`/`settings.rs`/`main.rs`/`ShortcutRecorder.tsx`/`lib/shortcut.ts`/`GeneralPane.tsx`/`WindowsPane.tsx`/`ShortcutBanner.tsx`/`styles.css`） | BETA-72 | 0.5d |
| **BETA-74** | **桌面自动更新（提前实现 V10-04）：轮询 GitHub Releases、左下角提醒、后台静默安装、装完自动重启**（用户要求：每 8 小时检查一次，后改为每 4 小时且做成可配置项，最短 30 分钟最长 24 小时；点「更新」后台下载安装、保留所有配置/数据/MCP token、只更新程序包，装完自动打开） | **done（2026-08-09）**：**技术方案拍板轻量自研**、不接 `tauri-plugin-updater`——官方插件需要生成新 ed25519 签名密钥对存成 GitHub secret，还要改两个 Release workflow 生成合并 `latest.json`；两个 workflow 一律标 `prerelease: true`，GitHub `/releases/latest` 别名不可用，得另建固定 tag 专门托管 manifest，工作量与对发布流水线的改动明显更大（已用 AskUserQuestion 与用户核实、user 选轻量自研）。改为直接调 GitHub Releases API 找最新 tag，下载既有的 `Scout_{version}_aarch64.dmg` / `Scout_{version}_x64-setup.exe` 资产静默安装，不改任何 CI/workflow、不引入新 secret。**关键发现**：`nsis/uninstall-hooks.nsh` 里已有 `$UpdateMode` 守卫——NSIS 检测到"覆盖安装同一产品"会跳过所有数据清理分支，静默重跑新版 `*-setup.exe` 本来就是官方支持的原地升级路径，settings.json / index.db / models / MCP token 全部自动保留，无需另写保留逻辑；macOS 用户数据本就在 `scout_data_dir()`、与 `.app` 程序包分离，替换程序包天然不碰。**新增 `update.rs`**（镜像 `model_download.rs` 既有约定：reqwest stream 下载 + 进度 event、`AtomicBool` in-flight 守卫）：后台循环启动 30 秒后首检，随后按设置间隔轮询；macOS 侧挂载 DMG → 拷贝新 `.app` 到安装目录（先拷 `.new`、成功才 rename 替换，失败旧 app 完全不动）→ `open -n` 拉起新版本 → 自身 `exit(0)`；Windows 侧写临时 `.bat` 串起「等自身退出 2s → 静默装 `/S` → 拉起新进程 → 自删」、spawn 后立即 `exit(0)` 释放文件锁。**设置项**（用户中途追加需求）：`settings.rs` 新增 `auto_update_enabled`（默认开）+ `auto_update_interval_minutes`（默认 240 分钟=4 小时，读取时 clamp [30,1440]），`update.rs` 后台循环每轮 live-read 这两个值（与 `auto_index_interval_minutes` 既有 live-read 约定一致，关闭时 60 秒重读一次、不停循环）；「常规」设置页新增开关 + 检查间隔下拉（30分钟/1/2/4(默认)/6/8/12/24 小时），关总开关时间隔选择器联动禁用。**前端**：`UpdateToast.tsx` + `useAutoUpdate.ts`，左下角 toast 四态（发现新版本/下载中/安装中/失败），失败态给"到 GitHub Releases 页手动下载"兜底文案。**验证**：Rust 新增 12 个单测全绿（版本比较/资产平台匹配/mock GitHub 响应解析/设置默认值与 clamp 边界），`cargo test -p scout-desktop` 211 全绿，`clippy -D warnings`/`fmt --check` 在 macOS 与 `--target x86_64-pc-windows-gnu` 交叉编译两个目标均净；`tsc`/`vite build` 全绿；浏览器预览注入 `window.__TAURI_INTERNALS__.invoke`/事件 stub，逐一截图验证左下角提醒四态定位与交互、设置页开关联动禁用、`update_settings` 保存回传正确值；针对真实 `api.github.com/repos/huibinma/Scout/releases` 拉取确认资产命名与匹配规则一致。bump v0.9.50，push + tag，CI/Release macOS/Release Windows 三个 workflow 均已触发并等待完成确认。**未尽事宜**：未做真机验证（尤其 Windows 静默安装脚本的实际文件锁释放时序、macOS `/Applications` 写权限边界情况）；没有加"手动检查更新"按钮、也没做失败时的权限提升重试，均超出用户原始需求范围，保持最小实现；这条能力需要下一个真实版本发布后才能做端到端"发现新版本"真机回归（开发环境测不出该真实网络路径）。 | apps/desktop（新增 `update.rs`/`UpdateToast.tsx`/`useAutoUpdate.ts`；改 `main.rs`/`settings.rs`/`App.tsx`/`GeneralPane.tsx`/`useAppSettings.ts`/`styles.css`） | BETA-73 | 1d |
| **BETA-75** | **v0.9.54 Windows/“找文件”真机缺陷收口：定位文件夹、路径前缀、内容命中大小、MSVCP140.dll 闪退** | **done（2026-08-11，v0.9.54 已发布）**：①“在文件夹中显示”改为 `explorer.exe` 的 `/select,` 与目标路径分参，并在 open/locate 前剥离 Windows verbatim 前缀，修复 Explorer 把整串误解析后打开错误目录；② common 新增 `strip_windows_verbatim_prefix`/`user_facing_path`，桌面结果、预览候选与 MCP provenance 统一隐藏 `\\?\`/`\\?\UNC\`，内部索引路径不改；③ common 新增 `file_size_bytes`，LocalIndex 内容/OCR/音乐命中与 SemanticIndex 命中均从 metadata 补 `size_bytes`，不再只有“文件名”匹配显示大小；④ `MSVCP140.dll` AppCrash 最高概率落在内嵌 llama.cpp C++ 路径——Windows loader 改为当前 exe 的常驻 helper 子进程，以 JSON-lines 承载 load/generate/embed，原生 abort/SEH 只令模型请求失败并走现有 parser/FTS 降级，不再带走桌面主进程；移除未使用的 `mtmd` feature；Windows Release 令 Rust、llama.cpp 与 Tauri 最终链接同用静态 MSVC CRT（`/MT`），并用 `dumpbin /DEPENDENTS` 闸门禁止 `MSVCP`/`VCRUNTIME` 动态导入回归。**验证**：新增路径/大小/IPC 协议回归测试；workspace fmt/clippy/build 净，沙箱外 desktop 210/210 全过；workspace 仅 `scoutd` 3 个既有正文读取 e2e 仍失败（与本轮模块无关、CI 不含该 binary e2e）；Windows GNU 条件编译覆盖 desktop 的 `model-fallback,semantic-recall`，model-runtime Windows 全目标 clippy 通过；macOS model-runtime llama tests 31 pass（3 个真模型测试按设计 ignored）；synonym recall 100%/FP 0%，tsc/vite build 通过。**边界**：无用户机器 crash dump，归因为基于 faulting module、release 链接配置与仓库既有 llama native crash 记录的高概率判断；v0.9.54 Windows MSVC/NSIS Release CI（run 31471683431）已通过，`dumpbin /DEPENDENTS` 确认最终 EXE 不导入 `MSVCP*` / `VCRUNTIME*`；仍需原问题 Windows 真机复现路径确认。 | apps/desktop + packages/model-runtime + packages/search-backends/{common,local-index,semantic-index} + packages/harness + packages/scout-server + release-windows.yml | BETA-74 | 1d |
| **BETA-76** | **重构：移除外部 Everything 依赖，内置 MFT/USN 原生索引服务替代**（用户 `/goal` 驱动：① 去掉对外部 Everything 的集成与依赖；② 内置一个实现”everything”索引的服务，涵盖 MFT 枚举、内存索引结构、USN Journal 实时监控，做到低资源占用、极速文件元数据检索） | **done（2026-08-20，随 v0.9.55 发布并完成本地验收）**：新增 `packages/search-backends/native-index`（`scout-native-index`）——`sys.rs`（全 crate 唯一含 `unsafe` 的模块，crate 级放开 `unsafe_code`，其余 workspace 仍受 `forbid` 约束）封装 `CreateFileW` 打开卷句柄 + `DeviceIoControl` 调 `FSCTL_ENUM_USN_DATA`（全量 MFT 枚举）/`FSCTL_QUERY_USN_JOURNAL`/`FSCTL_READ_USN_JOURNAL`（USN Journal 增量 tail，独立后台线程轮询，`Drop` 时 `join`——吸取 BETA-64 §5.2 T9a 教训，不 join 会有资源竞态）；`record.rs` 纯函数按偏移量解析 `USN_RECORD_V2` 原始字节（不靠 unsafe 转型）；`index.rs` `MemIndex`：`FRN → 记录` 扁平 `HashMap` + 父子链路径重建 + 子串/扩展名/组合查询（`NameQuery`：关键词组间 AND、组内 OR），刻意不做倒排索引/分词——文件名总量级下线性扫描已够快，且没有额外索引结构的内存/维护开销，这是"低资源占用"的来源，也是 Everything 实际采用的技术路线；`backend.rs` 实现 `SearchBackend`（新增 `BackendKind::NativeFileIndex`，移除 `BackendKind::Everything`），承接原 Everything 纯文件名查询角色，位置/时间/大小约束走结果端 `fs::metadata` 过滤（索引本身只存文件名，不存 metadata，用候选上限防苛刻条件下扫全卷）。**移除**：`packages/search-backends/everything` 整个 crate 删除；`packages/indexer/src/discovery.rs` 的 `EverythingDiscovery`/`EverythingExtDiscovery` 改为 `NativeIndexAudioDiscovery`/`NativeIndexExtDiscovery`；desktop `settings.rs` 的 `enable_everything` 改名 `enable_native_file_index`（`#[serde(alias = "enable_everything")]` 兼容旧字段，防用户升级后开关静默复位为默认值）；`permissions.rs`/`model_download.rs`/`main.rs` 对应改名（`check_native_file_index_available`/`ScanBackend::NativeFileIndex` 等）；前端 `EverythingPane.tsx`/`EverythingCheckStep.tsx` 分别改名 `NativeIndexPane.tsx`/`NativeIndexCheckStep.tsx`，UI 文案从"安装引导（winget/官网下载）"改为"检测到不可用时提示以管理员权限运行"（Win32 打开卷句柄的系统硬性要求，非本实现选择）。**已知限制**：仅支持 NTFS + `USN_RECORD_V2`（ReFS/128-bit FileId 的 V3 记录跳过，NTFS 卷现状恒为 V2）；打开卷句柄需管理员权限，本机非管理员会话下已验证优雅降级路径（真实报错 `VolumeOpen`「拒绝访问」，指数级回退目录扫描）。**验证**：本机 Windows 11 实机（非沙盒，真 `cargo`/`node` 工具链）`cargo check/clippy -D warnings/test/fmt --check --workspace` 全绿（仅 2 个既有 `scout-platform-macos` 测试因本机是 Windows 而非 macOS 失败，与本轮改动无关、`git diff` 确认零改动）；`scout-native-index` 28 单测全绿 + 1 个 `--ignored` 真机冒烟（非管理员下验证了优雅降级分支）；桌面 `tsc --noEmit` + `vite build` 通过；`docs/third-party-licenses.md` 已同步（移除 Everything/voidtools 条目，登记 `windows` crate 为正式依赖）。**未尽事宜**：管理员权限下的完整真机功能验证（全盘枚举正确性、USN tail 实时性与延迟、搜索结果质量、与 Windows Search/本地索引的 fan-out 协同）——留待下一轮真机测试；`docs/index-performance-design.md` 等历史设计记录按项目惯例未改写（保留原 Everything 决策的历史记录，只在受影响的当前状态类文档如 PROJECT.md/README.md/windows-setup.md/third-party-licenses.md 更新）。**v0.9.55 发布收口（2026-08-20）**：CI 首次因 Linux runner 编译到本 crate 而红——`record.rs`/`service.rs` 里只在 `#[cfg(windows)]` 生产路径引用的代码在非 Windows 判定死代码/未用 import，本机 Windows clippy 原理上看不到这类问题；修复后 CI/Release macOS/Release Windows 三个 workflow 全绿；本机真实安装 v0.9.55 验证 `native_file_index` backend 注册成功、非管理员会话按设计优雅降级（`available=false`），无崩溃；管理员权限下的真实功能仍未验证（当前会话无法交互式弹 UAC 提权）。 | packages/search-backends/{native-index(新增),everything(删除),common} + packages/indexer + packages/harness + packages/ranker + apps/desktop + apps/scout-cli + docs/{third-party-licenses,windows-setup,search-intent-schema}.md + PROJECT.md/README.md/CONVENTIONS.md/PRIVACY.md | BETA-75 | 1d |
| **BETA-77** | **重构：找文件搜索框"快速查找 + 深度检索"双模式 + 启动期原生索引后台预热**（用户 `/goal` 驱动，紧接 BETA-76：① 搜索框输入时按元数据索引动态出结果（类 Everything"快速查找"），回车触发元数据+语义索引"深度检索"；② desktop 启动时元数据索引常驻内存实现极速启动，语义索引等重资源后台准备、不占用启动瞬时资源/不卡顿） | **done（2026-08-20，随 v0.9.55 发布；桌面 GUI 交互仍待人工真机复测）**：新增 `apps/desktop/src-tauri/src/search/quick.rs`：`quick_search_impl` 跳过 NL intent 解析 / policy / 同义词扩展的完整管线，直接从 `ToolRegistry` 按 id 取 `search.local`（本地 SQLite FTS）/`search.native_file_index`（BETA-76 原生索引）两个已注册 `SearchableTool`，构造最小 `FileSearch` intent 并发调 `SearchBackend::search()`、按路径去重合并、按匹配紧密度（相等 > 前缀 > 子串）粗排——零新增数据访问代码，纯复用已验证的 backend。`#[tauri::command]` thin wrapper `search::quick_search` 留在 `search.rs`（与 `search`/`get_preview` 等同一模式，因 `generate_handler!` 宏要求隐藏 sibling 项与命令函数同模块，属性不能下沉到子模块再 `pub use`）。前端 `SearchView.tsx` 新增 120ms 防抖下拉（`.quick-results-dropdown`，复用 `.history-dropdown` 定位样式）：`quickResults`/`showQuickResults`/`inputFocused` 状态 + 防旧请求覆盖的请求代次 + 回车/Escape/点击结果三条隐藏路径。`main.rs` `.setup()` 新增 native-index 后台 `spawn_blocking` 预热（触发 `scout_native_index::native_index_available()`，fire-and-forget，不阻塞窗口显示）；语义索引后台准备经代码走读确认**架构已支持**（`EmbeddingModelHandle::new()` 惰性 + 既有 `spawn_semantic_index` 后台管线），本轮零改动。**真机浏览器验证抓到并修复一个真实竞态 bug**：回车提交深度检索、`setShowQuickResults(false)` 后，若 input 很快重新聚焦（真实交互中常见），防抖 `useEffect` 因依赖数组含 `inputFocused` 而重跑，120ms 后把刚被回车关掉的快速查找下拉又弹回来——用 `suppressQuickRef`（回车置位、`onChange` 清除）从根上压制，并顺带修了一个次要的 `onBlur` 定时器竞态（`blurTimerRef` 在 `onFocus` 时清掉待执行的旧隐藏定时器，防止它在重新聚焦之后才触发）。**验证**：workspace `cargo check/clippy -D warnings/test/fmt --check` 全绿（`scout-desktop` 211 测试，较 BETA-76 后 +2）；桌面 `tsc --noEmit`/`vite build` 通过；用 Vite dev server + `window.__TAURI_INTERNALS__.invoke` stub（临时改 `index.html` 内联脚本，验证后已还原、`git diff` 确认零残留）在浏览器里做了真实点击/输入/键盘交互验证：防抖出现、Escape 隐藏、点击结果触发 `open_path` 并关闭下拉、回车压制不回弹（改前复现问题、改后验证修复）。**未尽事宜**：`quick_search` 在真实大卷/管理员权限场景下的实际响应延迟与结果质量（尤其 native-index 尚未完成后台预热时的首次查询体验）留待真机验证。**v0.9.55 发布收口（2026-08-20）**：CI/Release macOS/Release Windows 三个 workflow 全绿（见 BETA-76 收口记录）；本机真实安装启动验证后台预热逻辑按设计执行、无异常日志；本轮无原生桌面自动化工具，快速查找下拉/回车切深度检索的真机点击交互仍未做端到端复测（开发期已用浏览器 stub 注入方式验证过前端逻辑本身）。 | apps/desktop/src-tauri/src/{search.rs,search/quick.rs,main.rs} + apps/desktop/src/{SearchView.tsx,styles.css} | BETA-76 | 0.5d |
| **BETA-78** | **重构：Scout 拆分为后台 Windows Service（scoutd）+ 前端瘦客户端桌面**（用户 `/goal` 驱动：读取 NTFS MFT〔BETA-76 内置原生索引〕依赖管理员权限，桌面普通用户权限运行满足不了这个前提；要求把索引构建/更新/检索/MCP 服务整体挪进一个以足够权限常驻的后台 service，desktop 降级为面向普通用户的检索 GUI client；安装时自动装好配置好 service 并自动启动，desktop 启动后自动连接） | **done（v0.9.56 已发布，CI/Release 三个 workflow 全绿；管理员权限下的真实安装+SCM 端到端验证未做，见下）**：**架构**：`scoutd` 新增个人模式（`apps/daemon/src/{personal,service}.rs`）——`bootstrap-personal-config`（幂等生成单 collection config.toml + token + connection.json，默认 roots=Desktop/Documents/Downloads/Pictures/Music，bind 强制 `127.0.0.1:8765` loopback-only，区别于团队模式默认 `0.0.0.0`）、`install-service`/`uninstall-service`（`windows-service` crate 走标准 SCM 注册，LocalSystem 账户，`StartPending`+checkpoint 覆盖 SCM 30s 启动超时风险）、`service`（真正的 SCM 入口）三个新子命令，不给子命令时今天的团队部署前台行为完全不变（零迁移）。`packages/scout-server` 新增 `POST /search`（完整 NL 管线，`SearchTool::invoke` 核心逻辑抽成 `execute_search` 供 MCP/REST 共用）、`POST /search/quick`（精简检索，桌面快速查找下拉用）、`POST /backend/search`（**桌面瘦客户端集成的关键接口**——单 backend 检索代理，桌面保留自己完整的本地 harness 管线〔policy/refine/同义词/路由/fan-out/tracer 全不变〕，只是 `search.local`/`search.semantic`/`search.native_file_index` 三个 backend 改由 `RemoteSearchBackend`〔`apps/desktop/src-tauri/src/service_client.rs`〕经这个端点向 scoutd 借一次 `search_expanded()` 调用；`search.native_file_index` 卷范围不受 collection 限制、只有 admin token 能触达，防企业限权 token 借此绕过信息墙）、`GET /admin/status`、`POST /admin/personal/roots`（落盘生效，因 `CollectionRuntime.meta.roots` 当前是构造期定值、无运行时热更新路径，改根目录需服务重启，`restart_required` 字段告知调用方）。桌面 `build_registry()` 三个 backend 换成 `RemoteSearchBackend`（`search.semantic` 额外注入相似度下限过滤——原在服务端 backend 内部执行，挪到客户端拿到结果后 filter，行为不变）；新增 `ServiceConnection`（`connection.json` 发现 + 5s 健康轮询 + 自动重连，desktop 启动即自动连接）。**安装器**（`apps/desktop/src-tauri/nsis/hooks.nsh`，原 `uninstall-hooks.nsh` 改名承载新增的 `NSIS_HOOK_POSTINSTALL`/`NSIS_HOOK_PREUNINSTALL`）：`installMode: perMachine`（NSIS 请求管理员权限）；装机自动跑 `bootstrap-personal-config`+`install-service`（均幂等）；卸载前先 `sc.exe stop`（释放文件锁，升级/真卸载都需要）、真卸载再 `sc.exe delete` + 沿用既有模型/索引保留询问一并覆盖 `%ProgramData%\Scout\scoutd`；`release-windows.yml` 新增用 `scout-model-runtime/llama-cpp` feature 编 `scoutd.exe`、经 `tauri build --config` **内联 JSON**（不写进 `tauri.conf.json` 本体——那是本地 `cargo check`/`tauri dev` 也读的基础配置，resources 引用的文件本地开发机不存在会导致所有人本地构建报错）注入 `bundle.resources`。**范围裁剪**（计划阶段已定）：只做 Windows（macOS 无本次要解决的 MFT 权限问题、且 Tauri macOS 打包是 DMG 无 post-install 钩子）；搜索流式进度收窄为"开始+结果"两段（不做 service 端逐 backend 分阶段推送）；桌面本地 FTS/语义后台 reindex 循环**保留未删**（`run_auto_index_loop` 等），继续维护桌面自己的 `index.db`——决策原因：`search/preview.rs` 的预览/OCR 片段展示依赖本地 index.db，若立即让桌面本地索引与 scoutd 索引各自独立漂移，会出现"搜到的文件预览不到"的新 bug；`mcp_service.rs`（桌面自带本机 MCP 服务）、`settings.rs` 索引 roots 编辑 UI、`reindex`/`reindex_root` 命令均**未改**，仍操作桌面本地冗余索引，不影响 scoutd 检索结果但也不会同步用户在设置页新增的目录到 scoutd——这些是明确的后续任务，非疏漏。**验证**：workspace `cargo check/clippy -D warnings/fmt --check/test`（`scoutd` 25 + `scout-server` 99 + `scout-desktop` 211，共 335 个测试）全绿；本机手动跑 `scoutd bootstrap-personal-config`+前台模式（非 SCM，本机非管理员会话无法测 `--install-service`/真 Windows Service）起真实服务、真实索引一个含中英文文件的测试语料，`curl` 逐一验证 `/health`/`/admin/status`/`/search`/`/search/quick`/`/backend/search` 五个端点返回正确命中（含 snippet/collection/score 字段）。**未尽事宜（下一轮真机验收清单）**：① 管理员权限下 `--install-service` 真实注册+`sc start`+SCM StartPending/Running 状态流转；② 真实 NSIS 安装包端到端（`installMode: perMachine` 触发 UAC、post-install/pre-uninstall 钩子真实调通、卸载钩子真实停删服务）；③ 桌面原生窗口下的搜索 UI 真机点击（本环境只有浏览器自动化、无法驱动 Tauri 原生窗口，且 stub 注入不会真正走 Rust IPC，等于测不到本轮改动）。**v0.9.56 发版收口（2026-08-20）**：bump 版本 → push main + tag → CI/Release macOS 一次过，**Release Windows 首次失败**——`tauri-action` `args` 里内联 JSON 传 `--config`（`"{\"bundle\":...}"`）经 GitHub Actions YAML 纯量传递，`\"` 未被当转义引号处理、action 内部按空白拆 argv 时把 `{\` 拆成独立 token 当文件路径去找，报路径不存在；改成 CI 里先写一个真实 JSON 文件（`apps/desktop/tauri.beta78.patch.json`）、`--config` 传文件路径彻底绕开转义问题，移到耗时的 llama.cpp 步骤之前（配置错误几秒内暴露）；移动 tag 重新触发，三个 workflow 全绿。真实 `tauri build` 日志核实 `scoutd.exe` 已正确打进 NSIS 包（Windows 安装包 7.94MB→14.56MB，新增体积核实即为打包进去的 llama-cpp 版 `scoutd.exe`，已向用户说明非异常）；`gh release edit` 已补真实 changelog。管理员权限下的真实安装+服务注册仍未验证（当前会话无法交互式弹 UAC）。 | apps/daemon（新增 personal.rs/service.rs + cli.rs/main.rs 改造）+ packages/scout-server（新增 quick_search.rs/search_http.rs + admin.rs/app.rs/collections.rs/tools/search.rs 改造）+ apps/desktop/src-tauri（新增 service_client.rs + main.rs 改造）+ apps/desktop/src-tauri/nsis + .github/workflows/release-windows.yml + apps/daemon/README.md | BETA-76 | 2d |
...
| V10-04 | 自动更新机制 | **done（2026-08-09，提前在 B 阶段实现，见 BETA-74）**：未走 Sparkle/Squirrel，改为轻量自研——App 定期调 GitHub Releases API、直接下载既有安装包静默安装，理由与实现细节见 [BETA-74](#33-b-阶段beta) | apps/desktop（`update.rs`） | done |
...
| 指标 | 阈值 | 数据集 / 验证方式 |
|---|---|---|
| 自动更新机制 | done（2026-08-09，见 BETA-74） | 轻量自研（GitHub Releases 轮询 + 静默安装），非 Sparkle/Squirrel；端到端真机回归待下个真实版本发布后补做 |
