...
> 本文件是工程侧细则，逐步填充。详细原则见 [Scout项目注意事项与风险清单.md](./Scout项目注意事项与风险清单.md)。

## 已落地

### 操作审计日志（BETA-06 Audit Log，2026-06-02）

Scout 对文件执行的操作（open/locate/copy/move/rename）记录一条审计：时间、操作、源路径、
目标、结果。设计原则：

- **本地优先**：append-only JSONL 存 `<data_dir>/Scout/audit.jsonl`，**永不上传**、不进任何
  telemetry。
...
用户在搜索框教给 Scout 的词汇映射（如「友商竞争分析 → AWS / Azure / …」）。设计原则：

- **本地优先**：存 `app_config_dir/Scout/user-synonyms.yaml`，**不上传、不同步**。
- **可查看可管理**：设置页「我的同义词」可查看 / 编辑 / 删除 / 导出。
- **不进 telemetry**：用户词典内容不进 trace（默认关闭；`SCOUT_TRACE` 开启时可见，便于 dev 调试）。
